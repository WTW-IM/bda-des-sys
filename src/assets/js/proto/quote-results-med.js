$(() => {
  ...
  $.getJSON('/assets/js/proto/medicare.json', data => {
    $.each(data.plans, function (i) {
      ...
      var _starFill = ((_rating / 5) * 100);
      var _oop = this.oop;
      var _drPcp = this.doctor.pcp;
      var _drSpecialist = this.doctor.specialist;
      var _rxGenericCopay = this.drugs.generic.toFixed(2);
      var _rxBrandCopay = this.drugs.brand.toFixed(2);
      var _hospital1 = this.hospital.tier1;
      var _hospital2 = this.hospital.tier2;
      var _hospital3 = this.hospital.tier3;
      ...
      var _drugsCovered = _hmcDrugs.reduce((n, drug) => n + (drug.isCovered == true), 0);

      // Calculate number of doctors covered by each plan
      var _docsCovered = _hmcDocs.reduce((n, doc) => n + (doc.isCovered == true), 0);
      ...
      var _docsBarClass = function () {
        if (_docsPct <= 10) {
          return 'drs10';
        } if (_docsPct > 10 && _docsPct <= 20) {
          return 'drs20';
        } if (_docsPct > 20 && _docsPct <= 30) {
          return 'drs30';
        } if (_docsPct > 30 && _docsPct <= 40) {
          return 'drs40';
        } if (_docsPct > 40 && _docsPct <= 50) {
          return 'drs50';
        } if (_docsPct > 50 && _docsPct <= 60) {
          return 'drs60';
        } if (_docsPct > 60 && _docsPct <= 70) {
          return 'drs70';
        } if (_docsPct > 70 && _docsPct <= 80) {
          return 'drs80';
        } if (_docsPct > 80 && _docsPct <= 90) {
          return 'drs90';
        } if (_docsPct > 90 && _docsPct <= 101) {
          return 'drs100';
        }
      };
      ...
      var _el = '<li data-carrier="' + _carrier + '" data-premium="' + _premiumDollars + '.' + _premiumCents + '" data-rating="' + _rating + '" data-doctors="' + (_docsPct / 10) + '" data-annual="' + _annualData + '">'
        ...
        + '    <span class="detail-row clearfix">'
        + '      <ol class="clearfix">'
        + '        <li><span class="icon medical" aria-label="includes medical coverage" title="Includes medical coverage"></span>'
        + '        <li><span class="icon rx" aria-label="includes prescription coverage" title="Includes prescription coverage"></span>'
        ...
        + '              </g>'
        + '              <g class="mask">'
        + '                <rect class="bar" x="0" y="0" display="inline" style="width: ' + _starFill + '%"/>'
        ...
        + '      <span class="icon medical" aria-label="includes medical coverage" title="Includes medical coverage"></span>'
        + '      <span class="icon rx" aria-label="includes prescription coverage" title="Includes prescription coverage"></span>'
        + '      <span class="sr-only">Network type:</span> <span class="network-type h4"><button type="button" class="popover-link" data-details="network-type">' + _networkType + '</button></span>'
        + '    </span>'
        + '    <span class="detail-row">'
        + '      <span class="star-rating">'
        + '        <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 156 27" enable-background="new 0 0 156 27" xml:space="preserve">'
        ...
        + '    <!-- help-me-choose -->'
        + '    <span class="hmc">'
        + '      <h6 class="h5"><button type="button" class="popover-link" data-toggle="popover" data-trigger="focus" data-placement="right" data-html="true" data-container="#plan-tiles" data-content="<p>Your Annual Cost&#8480; brings together information, such as:</p><ul><li>an estimate of your <strong>annual costs</strong> under this plan</li><li>how many of <strong>your doctors</strong> accept this plan</li><li>how many of <strong>your prescriptions</strong> are covered by this plan</li></ul><p>to help you decide if this plan is right for you and your budget.</p>" title="Your Annual Cost&#8480;">Your Annual Cost&#8480;</button> for this Plan</h6>'
        ...
        + '      </span>'
        + '      <button type="button" class="popover-link medicare-prem-det-' + i + '" data-details="plan-premium" title="Plan Premiums for a Year: $' + (_premium * 12).toFixed(2) + '">Premium over twelve months: $' + (_premium * 12).toFixed(2) + '</button>'
        + '      <button type="button" class="popover-link medicare-prem-det-' + i + '-bar" data-details="plan-premium" title="Plan Premiums for a Year: $' + (_premium * 12).toFixed(2) + '">Premium over twelve months: $' + (_premium * 12).toFixed(2) + '</button>'
        + '      <button type="button" class="popover-link medicare-rx-det-' + i + '" data-details="drugs-covered" data-container="#plan-tiles" title="Drug Costs for a Year: $XXX.XX">' + _drugsCovered + ' of ' + _totalDrugs + ' drugs</button>'
        + '      <button type="button" class="popover-link medicare-rx-det-' + i + '-bar" data-details="drugs-covered" data-container="#plan-tiles" title="Drug Costs for a Year: $XXX.XX" aria-hidden="true">' + _drugsCovered + ' of ' + _totalDrugs + ' drugs</button>'
        ...
        + '    <thead>'
        + '      <tr>'
        + '        <th><button type="button" class="popover-link" data-toggle="popover" data-trigger="focus" data-placement="top" data-html="true" data-container="#plan-tiles" data-content="<p>Care in a hospital that requires admission as an inpatient and usually requires an overnight stay. Includes all services that Medicare Part A covers such as a semi-private room, meals, regular nursing services, and other hospital services and supplies.</p>" title="Inpatient Hospital Stay">Inpatient Hospital</button></th>'
        ...
        + '        <th><button type="button" class="popover-link" data-toggle="popover" data-trigger="focus" data-placement="top" data-html="true" data-container="#plan-tiles" data-content="<p>The amount you will pay for prescription drugs. Some Medicare plans have different levels, or &ldquo;tiers&rdquo;, of drugs, with different costs (copayments or coinsurance) at each level.</p>" title="Prescription Drug Coverage">Prescription Drugs</button></th>'
        + '      </tr>'
        + '    </thead>'
        + '    <tbody>'
        + '      <tr>'
        + '        <td><button type="button" class="popover-link" data-toggle="popover" data-trigger="focus" data-placement="bottom" data-html="true" data-container="#plan-tiles" data-content="<ul><li>' + _hospital1.description + ': $' + _hospital1.copay + ' copay/day' + '</li><li>' + _hospital2.description + ': $' + _hospital2.copay + ' copay/day' + '</li><li>' + _hospital3.description + ': $' + _hospital3.copay + ' copay/day' + '</li></ul><small>Services may require prior authorization and/or a referral from your doctor.</small>" title="Inpatient Hospital Stay">$' + _hospital1.copay + ' / $' + _hospital2.copay + ' / $' + _hospital3.copay + '</button></td>'
        + '        <td>$' + _oop + '/year</td>'
        + '        <td><button type="button" class="popover-link" data-toggle="popover" data-trigger="focus" data-placement="bottom" data-html="true" data-container="#plan-tiles" data-content="<ul><li>Primary Care Physician: $' + _drPcp + ' copay</li><li>Specialist: $' + _drSpecialist + ' copay</li></ul><small>For copayments for other health care providers, please see plan details.</small><small>Services may require prior authorization and/or a referral from your doctor.</small>" title="Doctors&rsquo; Office Visits">$' + _drPcp + ' / $' + _drSpecialist + '</button></td>'
        + '        <td><button type="button" class="popover-link" data-toggle="popover" data-trigger="focus" data-placement="bottom" data-html="true" data-container="#plan-tiles" data-content="<ul><li>Preferred Generic Drugs: $' + _rxGenericCopay + ' copay</li><li>Preferred Brand Drugs: $' + _rxBrandCopay + ' copay</li></ul><small>For copayments for other tiers of drugs under this plan, please see the plan details.</small>" title="Prescription Drug Coverage">$' + _rxGenericCopay + ' / $' + _rxBrandCopay + '</button></td>'
        + '      </tr>'
        + '    </tbody>'
        + '  </table>'
        ...
        + '      </label>'
        + '    </div>'
        + '    <button type="button" data-trigger="addToCart" class="btn btn-primary-alt pull-right">Add to Cart <span class="icon double-chevron-right"></span></button>'
        + '  </span>'
        + '</li>';
      _results.append(_el);
    });
